...
namespace ScaryHouse
{
    public partial class Form1 : Form
    {
        ...
        private void UpdateGridCellSeen(int row, int col)
        {
            ...
            // publish play message for room cells (columns >= 2)
           /* if (col >= 2)
            {
                int group = row + 1;
                int roomNo = col - 1; // columns: 0=Group,1=maingate,2=01,...
                string topic = $"music/group{group}";
                string payload = $"play,{roomNo.ToString("D2")}";
                _ = Task.Run(async () => { try { await PublishIfConnected(topic, payload, false); } catch { } });
            }*/
        }
