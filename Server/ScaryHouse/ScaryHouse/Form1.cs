...
namespace ScaryHouse
{
    public partial class Form1 : Form
    {
        ...
        private void HandleEachGateMessage(string payload)
        {
            ...
            // Enforce fixed lag policy: previous-activated group must be ahead by at least Lag gates
            // Determine previous group (immediately earlier in activation order) for the targetGroup
            int prevGroup = -1;
            ...
            {
                int prevProgress = GetNextExpectedForGroup(prevGroup) - 1;
                int followerProgress = GetNextExpectedForGroup(targetGroup) - 1;
                if (prevProgress - followerProgress < Lag)
                {
                    Logger.Log($"EachGate {val} for Group {targetGroup} denied - previous Group {prevGroup} progress {prevProgress} too close to follower {followerProgress}");
                    // Try to reassign to earlier groups (starting from most recent previous) if any expects this gate and is allowed
                    ...
                    foreach (var g in ordered)
                    {
                        ...
                        if (pg != -1)
                        {
                            int prevgProgress = GetNextExpectedForGroup(pg) - 1;
                            int gProgress = GetNextExpectedForGroup(g) - 1;
                            if (prevgProgress - gProgress < Lag) continue; // not allowed
                        }
