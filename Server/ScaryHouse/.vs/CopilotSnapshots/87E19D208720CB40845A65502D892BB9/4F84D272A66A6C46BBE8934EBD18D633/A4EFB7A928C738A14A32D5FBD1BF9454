...
namespace ScaryHouse
{
    public partial class Form1 : Form
    {
        ...
        private async void Form1_Load(object sender, EventArgs e)
        {
            ...
            // attempt to pre-fill broker with local Wi-Fi IPv4 address if available
            try
            {
                var localIp = GetLocalWifiIpv4();
                if (!string.IsNullOrWhiteSpace(localIp))
                {
                    textBoxBroker.Text = localIp;
                }
            }
            catch { }
        ...
        private string GetLocalWifiIpv4()
        {
            try
            {
                ...
                {
                    // prefer wireless interfaces that are up
                    if (ni.NetworkInterfaceType == NetworkInterfaceType.Wireless80211 && ni.OperationalStatus == OperationalStatus.Up)
                    {
                        var ipProps = ni.GetIPProperties();
                        var uni = ipProps.UnicastAddresses.FirstOrDefault(a => a.Address.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork);
                        if (uni != null)
                        {
                ...
                // fallback: first active IPv4 address on any interface
                foreach (var ni in NetworkInterface.GetAllNetworkInterfaces())
                {
        ...
        private async void MqttClient_Disconnected(Exception ex)
        {
            // notify UI
            Invoke(new Action(() => buttonConnect.Text = "Connect MQTT"));
            ...
            try { host = textBoxBroker.Text; } catch { }

            while (!userRequestedDisconnect)
            {
                try
                {
                    // create a new client and attempt reconnect
                    ...
                    await client.SubscribeAsync(new[] { "status/group1", "status/group2", "status/group3" });
            ...
            try
            {
                ...
                for (int g = 1; g <= 3; g++)
                {
                    ...
                    if (groupMappings[g].ContainsKey(mac))
                    {
                        ...
                        Logger.Log($"Rejected status from {mac} for group {group} - already assigned to group {g}");
                        return;
                    }
                }
                ...
                if (globalIndex >= 0 && globalIndex < nodeLights.Count)
                {
                    ...
                    isOnline.TryGetValue(mac, out wasOnline);
                    ...
                    {
                        isOnline[mac] = true;
                        Logger.Log($"Device {mac} Group {group} HeadSet {slotIndex + 1} ONLINE battery {battery}%");
                    }
                }
            }
            catch { }
        ...
        private void buttonUpdateConfig_Click(object sender, EventArgs e)
        {
            using (var dlg = new RoomSettingsForm(roomConfig))
            {
                ...
                {
                    // roomConfig already saved by dialog
                    Logger.Log("Room timers updated");
                    MessageBox.Show("Room timers saved");
                }
            }
        }
        ...
        private void buttonReset_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < nodeLights.Count; i++)
            {
                ...
                batteryLabels[i].Text = $"HeadSet {slot}: -- | Battery: --%";
    ...
    internal class SimpleMqttClient : IDisposable
    {
        ...
        private async Task KeepAliveLoopAsync(CancellationToken ct)
        {
            try
            {
                // send ping at interval keepAliveSeconds/2
                ...
                while (!ct.IsCancellationRequested && IsConnected)
                {
                    ...
                    {
                        // will be handled by reader loop detecting disconnect
                    }
                }
            }
            catch { }
